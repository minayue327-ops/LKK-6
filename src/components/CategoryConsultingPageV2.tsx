...
import { ArrowRight, Sparkles, CheckCircle2, ChevronRight, ChevronDown, ChevronUp, Phone, QrCode, Star, Play, Pause } from 'lucide-react';
...
export default function CategoryConsultingPageV2({
  ...
  return (
    <div className="w-full bg-white">
      ...
      {/* 1.5 QUANTIFIED ACHIEVEMENTS - SECTION */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10">
            ...
            >
              ...
              <div className="p-6 sm:p-8 flex-grow flex flex-col justify-between">
                <div>
                  <h3 className="text-xl md:text-2xl font-bold text-[#111827] group-hover:text-[#007BC7] transition-colors mb-4 font-display">
                    三品合一品类咨询
                  </h3>
                  <ul className="grid grid-cols-2 gap-x-3 gap-y-2.5">
                    {['品类竞争', '用户洞察', '技术规划', '品类战略', '品类品牌', '品类产品', '品类营销'].map((b, i) => (
                      <li key={i} className="flex items-center gap-2 text-sm text-[#374151] font-normal leading-tight">
                ...
                <div className="mt-6 pt-5 sm:mt-8 sm:pt-6 border-t border-black/[0.06] flex items-center justify-between">
                  <span className="text-xs text-[#6B7280] font-semibold uppercase tracking-wider group-hover:text-[#007BC7] transition-colors">
                    了解详情
                  </span>
      ...
      <section id="service-matching" className="py-20 md:py-28 bg-[#FFFFFF] w-full border-b border-black/[0.06] relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6">
                <div className="flex items-center gap-4 text-left">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-[#6B7280] mt-1 font-normal">
                ...
                <div className="shrink-0 self-start sm:self-auto">
                  <a
                    ...
                    className="group inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#007BC7] hover:bg-[#005F96] text-white text-sm font-semibold transition-all duration-200 shadow-xs hover:shadow-md active:scale-[0.98] cursor-pointer select-none"
                    title="进入线上诊断"
                  >
                    ...
                    <button
                      ...
                      className={`px-5 sm:px-6 py-2 sm:py-2.5 rounded-full text-sm transition-all shrink-0 flex items-center gap-2 cursor-pointer ${
                        isActive
                          ...
                          : 'text-[#6B7280] hover:text-[#111827] font-medium bg-transparent'
                      }`}
                    >
                      {isActive && (
                          ...
                          <div className="w-full flex flex-col gap-4 sm:gap-5 text-left pb-5 sm:pb-6">
                            ...
                            <div className="w-full flex flex-col lg:flex-row lg:items-center justify-between gap-5 sm:gap-6">
                              ...
                              <div className="flex-1 max-w-xl flex flex-col justify-center">
                                ...
                                <div className="text-sm font-medium text-[#6B7280] tracking-wider mb-2">
                                  {item.id} · {item.subLabel}
                                </div>

                                {/* 样式 1: 30px (text-3xl) 痛点大标题 */}
                                <h3 className="text-3xl font-bold tracking-tight text-[#111827] mb-3 font-display">
                                  {item.problemTitle}
                                </h3>

                                {/* 痛点描述正文：字号与板块辅助说明完全一致 (text-sm md:text-base) */}
                                <p className="text-sm md:text-base text-[#4B5563] leading-relaxed max-w-lg font-normal">
                                  {item.description}
                                </p>
                              </div>
                            ...
                            <div className="w-full">
                              ...
                              <div className="text-sm font-medium text-[#6B7280] mb-2.5">
                                常见核心痛点
                              </div>
                              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3">
                                {item.questions.map((q, qIdx) => (
                                  <div 
                                    ...
                                    className="flex items-center gap-2.5 px-3.5 py-3 rounded-xl bg-[#F5F5F7] text-sm text-[#374151] font-medium"
                                  >
          ...
          <div id="category-advisory" className="max-w-[min(95%,1720px)] w-full mx-auto px-6 sm:px-10 lg:px-16 2xl:px-12">
            <div className="w-full bg-white rounded-3xl p-8 sm:p-12 lg:p-14 text-center relative overflow-hidden">
              <div className="relative z-10 max-w-3xl mx-auto">
                ...
                <h4 className="text-2xl sm:text-3xl lg:text-[32px] font-bold text-[#111827] font-display tracking-tight mb-3">
                  不确定当前所处的品类阶段与问题？
                </h4>
                <p className="text-sm sm:text-base text-[#4B5563] max-w-xl mx-auto leading-relaxed mb-8 font-normal">
                  与洛可可资深战略咨询顾问展开 1 对 1 诊断沟通，梳理企业业务现状与创新契机。
                </p>
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6">
                  <button
                    onClick={onOpenContactModal}
                    className="w-full sm:w-auto h-11 px-7 bg-[#007BC7] hover:bg-[#005F96] text-white text-sm font-semibold rounded-full transition-colors flex items-center justify-center gap-2 cursor-pointer shadow-xs"
                  >
                    <span>预约专家品类诊断</span>
                    <ArrowRight className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => onNavigateDetail?.('/three-in-one-category')}
                    className="w-full sm:w-auto h-11 px-4 bg-transparent hover:underline text-[#007BC7] text-sm font-semibold transition-colors flex items-center justify-center gap-1 cursor-pointer"
                  >
      ...
      >
          ...
          {faqItems.slice(0, 5).map((item, index) => (
            ...
            >
              ...
              <div className="max-w-[min(95%,1720px)] w-full mx-auto px-12 md:px-10 lg:px-24 2xl:px-12 py-6 flex flex-col text-left group">
                <h4 className="text-base sm:text-lg font-bold text-[#111827] group-hover:text-[#007BC7] transition-colors duration-300">
                  {item.q}
                </h4>
                <p className="mt-2.5 text-sm text-[#4B5563] leading-relaxed font-normal">
                  {item.a}
                </p>
              </div>
            </div>
        ...
        <div className="max-w-[min(95%,1720px)] w-full mx-auto px-12 md:px-10 lg:px-24 2xl:px-12 mt-10 md:mt-12 flex justify-center">
          <button
            ...
            className="group inline-flex items-center gap-2 px-8 py-3.5 rounded-full bg-[#F5F5F7] hover:bg-[#007BC7] text-[#111827] hover:text-white text-sm font-semibold transition-all duration-300 cursor-pointer border border-black/5 hover:border-transparent shadow-xs hover:shadow-md active:scale-[0.98]"
          >
            <span>查看更多</span>
